...
print('Recursive feature elimination',x_train.shape[1],'->',n_comp)
...
fs_rfe = feature_selection.RFE(estimator = SVR(kernel="linear"), n_features_to_select = int(n_comp))
## wyuczenie macierzy tranformacji i zaaplikowanie jej na x_train
start_time = time.time()



#x_train_reduced = fs_rfe.fit(x_train, y_train)
x_train_reduced = fs_rfe.fit_transform(x_train, y_train)
#fs_rfe_support = x_train_reduced.support_
#fs_rfe_ranking = x_train_reduced.ranking_

print("  took %s seconds " % round((time.time() - start_time),5))

#zastosowanie tej samej macierzy na x_test
x_test_reduced = fs_rfe.transform(x_test)

# zapisujemy dane
save_data(x_train_reduced, y_train, x_test_reduced, y_test, classes_names, output_dir)
# dodatkowo zapisujemy sam obiekt RFE

fs_rfe_object_file = open(output_dir+"/fs_rfe_object.pkl", "wb")
pickle.dump(fs_rfe,fs_rfe_object_file)